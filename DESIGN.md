...
The signature is one drawn line joining a spoken checkbox, a photo observation, a folded city map and a saved-task bookmark. These motifs come from Voice Todo, Jianwei, City Copy and Context Continuity. They are illustrations, not simulated product screenshots. Real app previews stay adjacent to the project they describe.
...
- Layout: compact identity image → native introduction/navigation → featured Voice Todo → paired product previews → continuity workflow → compact experiments → live updates.
...
The artwork and copy here are original. Reference artwork, characters and logos are not reused. An initial generic cream-paper direction was discarded in favor of blue observation notes and motifs tied to the actual featured projects.
...
## README refresh — 2026-09-17

Voice Todo now leads the profile. Its diagram uses Chinese commands from the implemented workflow and is explicitly labeled as an illustration. Each of the seven public project READMEs is organized around what it is, how to use it, and why it exists. Detailed guides remain linked; decoration is used only where it helps explain the product.

Additional editorial references: [LLM](https://github.com/simonw/llm) for a first useful command, [uv](https://github.com/astral-sh/uv) for concise setup, and [autoresearch](https://github.com/karpathy/autoresearch) for stating the motivation and execution boundary.
